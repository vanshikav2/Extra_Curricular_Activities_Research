...
```
### Campus Distance and Time Commitment
...
correlation <- cor(data$campusDistance, data$timeCommitment, use = "complete.obs")

# Create a scatter plot
ggplot(data, aes(x = campusDistance, y = timeCommitment)) +
  geom_point() +
  labs(title = "Scatter Plot of Campus Distance vs. Time Given",
       x = "Distance from Campus",
       y = "Time given") +
  theme_minimal()
...
```
The same can be seen in the correlation between Campus Distance and time commitment as the correlation coefficient is close to 0.

### Campus Distance and Student Involvement 
```{r}
#| echo: false
#| eval: true
#| warning: false
...
correlation <- cor(data$campusDistance, data$studentInvolvement, use = "complete.obs")

# Create a scatter plot
ggplot(data, aes(x = campusDistance, y = studentInvolvement)) +
  geom_point() +
  labs(title = "Scatter Plot of Campus Distance vs. Student Involvement",
       x = "Distance from Campus",
       y = "Number of Extracurricular Activities") +
  theme_minimal()
...
The correlation coefficient between Campus Distance and Student Involvement also show no significant relation.
